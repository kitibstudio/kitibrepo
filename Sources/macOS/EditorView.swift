...
/// Thin subclass whose only job is to intercept the `printDocument:` action
/// (`Cmd+P`) before `NSTextView` can run its own broken print path in a
/// SwiftUI context. The action is forwarded to our `Exporter` via the stored
/// closure; all other behaviour is inherited unchanged.
final class KitibTextView: NSTextView {
    var onPrint: (() -> Void)?
    ...
    /// `super.paste(_:)`. The coordinator sets this closure.
    var onPaste: ((NSPasteboard) -> Bool)?

    // NSTextView responds to the printDocument: action (Cmd+P via responder
    // chain). Override it here to route through our Exporter instead of
    // NSTextView's own print path, which crashes in a SwiftUI app with no
    // NSDocument. The selector is @objc because it is action-dispatched.
...
struct EditorView: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> NSScrollView {
        // Build the text system from scratch rather than stealing the
        // textContainer from scrollableTextView(). The theft pattern leaves
        // a stale layout manager attached to the container, and on current
        // macOS the bridge resets isEditable during hierarchy setup.
        ...
        let tv = KitibTextView(frame: .zero, textContainer: textContainer)
        tv.minSize = .zero
        ...
        tv.onPrint = { [weak coordinator] in
            coordinator?.triggerPrint()
        }
        tv.onPaste = { [weak coordinator] pasteboard in
    ...
    func updateNSView(_ scrollView: NSScrollView, context: Context) {
        ...
        // Re-assert editability after every SwiftUI layout pass — the
        // AppKit bridge can reset isEditable during view hierarchy setup.
        tv.isEditable = true
    ...
    final class Coordinator: NSObject, NSTextViewDelegate {
        ...
        private func updateCursorInTable() {
            ...
            let inTable = MarkdownTableParser.findTableRange(
                in: tv.string, cursorPosition: pos
            ) != nil
        ...
        func registerFind() {
            state.performFind = { [weak self] action in
                guard let tv = self?.textView else { return }
                // Map the platform-neutral verb to AppKit's find-bar action,
                // which NSTextView routes through a sender's tag.
                let nsAction: NSTextFinder.Action
                switch action {
                case .find:         nsAction = .showFindInterface
                case .replace:      nsAction = .showReplaceInterface
                case .next:         nsAction = .nextMatch
                case .previous:     nsAction = .previousMatch
                case .useSelection: nsAction = .setSearchString
                }
                let item = NSMenuItem()
                item.tag = nsAction.rawValue
        ...
        func registerTableGrid() {
            state.openTableGrid = { [weak self] in
                ...
                let tableText = (tv.string as NSString).substring(with: range)
        ...
        func registerOutline() {
            // Scroll the editor to a heading range.
        ...
        /// enters the document. Returns `true` when the preview sheet was
        /// shown (paste intercepted); returns `false` to fall through to the
        /// native paste path.
        func handlePaste(_ pasteboard: NSPasteboard) -> Bool {
            // Re-entry guard: if a preview is already open, fall through to
            // native paste rather than replacing the pending preview.
