...
3. Read **only** the source files that task touches.
4. Restate the task in one sentence with its acceptance criteria. **Wait for confirmation before writing code.**
...
- `/state/`, `/specs/`, `/docs/`, `/.Codex/`, `/captures/` are **not** in the Xcode target — never add them
...
Create a lightweight, minimalist native osx app to help writers focus.
